...
export function Onboarding({ onOpenChange }: { onOpenChange: (open: boolean) => void }) {
    ...
    <>
      ...
      {open && state ? (
        <div className="provider-wizard">
          <section className="provider-wizard__content" aria-labelledby="onboarding-title">
            ...
            <form className="provider-wizard__apikey-form" onSubmit={submit}>
              ...
              {error ? <p className="provider-wizard__error" role="alert">{error}</p> : null}
              <div className="provider-wizard__apikey-actions">
                <button className="provider-wizard__btn provider-wizard__btn--secondary" type="button" onClick={() => setOpen(false)}>Cancel</button>
                <button className="provider-wizard__btn provider-wizard__btn--primary" type="submit" disabled={saving || !provider || !model || Boolean(selected?.credential_name && !selected.configured && !credential.trim())}>
