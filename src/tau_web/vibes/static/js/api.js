 ...
 */
export const getTauDashboard = page => tau.dashboard(page);
export const getTauMeters = () => tau.meters();
 ...
 * Upload media file
 */
export async function uploadMedia(file, { signal, onProgress, sessionId } = {}) {
    onProgress?.(0);
    const result = await tau.upload(file, { signal, sessionId });
    onProgress?.(100);
    return result;
}
