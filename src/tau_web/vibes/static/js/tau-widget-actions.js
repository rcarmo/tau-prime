...
export function installTauWidgetActions({target=document,renderer=window.tauExtensionUI,client=createTauClient({getToken:()=>localStorage.getItem('tau.web.authToken')||''})}={}) {
 const handle=async event=>{
  ...
  try{
   ...
   renderer?.respondWidget(detail.frame_id,detail.request_id,result,true);
  }catch(error){renderer?.respondWidget(detail.frame_id,detail.request_id,error.message||'Widget action failed',false);}
 };
