/** Preserve a recovery copy before deleting durable queued input.
 * Storage failure must abort removal; later edits must never be rolled back.
 */
export function preserveQueuedRecovery(storage,{sessionId,queueId,text}){
 ...
 const key=`tau.queue-return:${encodeURIComponent(sessionId)}:${encodeURIComponent(queueId)}`;
