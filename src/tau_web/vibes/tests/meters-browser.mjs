...
try{
 ...
 for(const engine of [chromium,webkit]){
  ...
  try{
   const page=await browser.newPage({viewport:{width:390,height:844}});let requests=0,fail=false;const errors=[];
...
render(html\`<\${TauMeters}/>\`,document.getElementById('test'));
   ...
   assert.equal(await page.locator('.system-meters-row').count(),4);
   assert.equal(await page.locator('.system-meters-row.cpu path').getAttribute('d'),'M0.00,15.00 L56.00,11.50');
   const box=await page.locator('.system-meters-card').boundingBox();assert.ok(box.x>=0&&box.x+box.width<=390);
