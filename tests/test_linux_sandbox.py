...
for operation in [
    lambda: outside.write_text('bad'), lambda: outside.unlink(),
    lambda: os.truncate(outside, 0), lambda: (base/'new').mkdir(),
    lambda: (project/'ok').rename(base/'escaped'),
]:
    try: operation()
...
result = subprocess.run([
    sys.executable, '-c',
    'from pathlib import Path; import sys; Path(sys.argv[1]).write_text("bad")', str(outside),
], capture_output=True)
