...
test('message action copies original Markdown and announces clipboard failure', async ({ page }) => {
  ...
  await expect(page.locator('.message-action-bar [role="status"]')).toHaveText('Message copied');
  await page.getByRole('button',{name:'Collapse message',exact:true}).focus();
  await page.keyboard.press('Enter');
  await expect(page.locator('.message-list__collapsed-preview')).toHaveText(text);
  await expect(page.locator('.message-list__content')).toHaveCount(0);
  await expect(page.getByRole('button',{name:'Expand message',exact:true})).toBeFocused();
  await page.keyboard.press('Enter');
  await expect(page.locator('.message-list__content strong')).toHaveText('Original');
  ...
  await expect(page.locator('.message-action-bar [role="status"]')).toHaveText('Unable to copy message');
});
