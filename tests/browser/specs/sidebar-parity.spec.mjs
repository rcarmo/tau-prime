...
test('sidebar container matches actual Piclaw component markup and computed styles', async ({ page }) => {
  // Compile the genuine upstream component, not a Tau-generated reference.
  const result = await build({
    stdin: { contents: `import {render} from 'preact';
      import {Sidebar as Reference} from ${JSON.stringify(path.join(reference, 'components/Sidebar.tsx'))};
      import {Sidebar as Tau} from ${JSON.stringify(path.join(frontend, 'src/components/Sidebar.tsx'))};
      render(<Reference title="Workspace"><p>Shared fixture</p></Reference>,document.getElementById('reference'));
      render(<Tau title="Workspace"><p>Shared fixture</p></Tau>,document.getElementById('tau'));`, loader: 'tsx', resolveDir: frontend },
  ...
  // Tau's compatibility sheet must not alter the upstream sidebar's appearance.
  expect(await measure('#tau')).toEqual(upstream);
});
