...
if(p==='/workspace/tree')return json({root:{name:'workspace',path:'',type:'dir',children:[{name:'src',path:'src',type:'dir',children:[]},{name:'README.md',path:'README.md',type:'file'}]}});
...
await page.locator('.workspace-row').filter({hasText:'workspace'}).first().click();
await expect(page.locator('.workspace-tree-list')).toContainText('README.md');
